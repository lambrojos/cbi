...
var LogicalMessage = (function () {
    ...
    LogicalMessage.prototype.validate = function () {
        ...
        var array = _.pluck(this.paymentInfos, 'paymentInfoId');
        ...
        var lastLocalInstrument = null;
        for (var _b = 0, _c = this.paymentInfos; _b < _c.length; _b++) {
            var paymentInfo = _c[_b];
        ...
        var e2eIds = [];
        for (var _d = 0, _e = this.paymentInfos; _d < _e.length; _d++) {
            var paymentInfo = _e[_d];
            for (var _f = 0, _g = _.pluck(paymentInfo.directDebt, 'e2eId'); _f < _g.length; _f++) {
                var id = _g[_f];
                e2eIds.push(id);
            }
        }
